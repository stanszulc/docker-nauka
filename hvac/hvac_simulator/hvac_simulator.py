"""
hvac_simulator.py v7
====================
Zmiany v7:
- 35 urządzeń (SIM_01-35), wszystkie w trybie anomaly
- Dłuższy warmup: 180-400 ticków (30-67 min) zamiast 70-130 (12-22 min)
- FAILURE_TICKS_MIN = 18 (3 min) zamiast 12 (2 min)
- Usunięty tryb normal_only i pseudo-anomalie
...
# ── 35 urządzeń — wszystkie w trybie anomaly ──────────────────────────────────
DEVICES = [
    {
        'id':  f'SIM_{i:02d}',
...
FAILURE_TICKS_MIN = 18  # 18 × 10s = 180s = 3 minuty
...
class DeviceSimulator:
    def __init__(self, device_cfg):
        ...
        self.phase         = 'warmup'
        self.mode          = None
        # v7: dłuższy warmup 30-67 min gwarantuje wypełnienie WIN_LONG (10 min)
        self.warmup_left   = random.randint(180, 400) * TICK
...
def main():
    log.info("HVAC Simulator v7 | %d devices | server=%s | tick=%ds",
             len(DEVICES), SERVER_URL, TICK)
    log.info("Fault types: %s", FAULT_TYPES)
    log.info("Warmup: 180-400 ticks (30-67 min) | Failure hold: %d ticks = %ds",
