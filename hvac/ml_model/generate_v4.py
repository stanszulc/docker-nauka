...
def simulate_normal_sequence(seq_id, steps=None):
    """
    Sekwencja normalnej pracy — długa, z realistycznym szumem i pseudo-anomaliami.
    Pseudo-anomalie to krótkie wahania które NIE prowadzą do awarii (model uczy się normalności).
    Label=0 przez cały czas.
    """
    if steps is None:
        steps = np.random.randint(80, 200)  # długie sekwencje
    ...
    # Losowy dryf temperatury — symuluje zmiany otoczenia
    temp_drift = np.random.randn() * 0.3  # K/tick bias

    # Pseudo-anomalia — krótki spike który wraca do normy
    pseudo_start  = np.random.randint(20, steps - 20) if steps > 40 else -1
    pseudo_len    = np.random.randint(3, 8)  # 3-8 ticków
    ...
    for step in range(steps):
        ...
        # Dodaj dryf temperatury (realistyczny szum długoterminowy)
        ...
        # Pseudo-anomalia — krótki spike, NIE awaria
        in_pseudo = pseudo_start > 0 and pseudo_start <= step < pseudo_start + pseudo_len
        if in_pseudo:
            if pseudo_type == 'torque_spike':
                state['torque'] = clamp(state['torque'] + np.random.uniform(5, 15), R['torque']['min'], 58.0)  # max 58 — poniżej progu CLOG=60
            elif pseudo_type == 'rpm_drop':
                state['rpm'] = clamp(state['rpm'] - np.random.uniform(50, 150), 1400.0, R['rpm']['max'])  # min 1400 — powyżej progu HDF=1380
            elif pseudo_type == 'vibration_bump':
                state['vibration'] = clamp(state['vibration'] + np.random.uniform(0.1, 0.4), R['vibration']['min'], 0.75)  # max 0.75 — poniżej progu BEARING=0.8
...
def main():
    ...
    parser.add_argument('--output',    type=str, default='hvac_training_v4.csv')
    ...
    # 60% sekwencji to normalna praca z szumem i pseudo-anomaliami
    # Model musi nauczyć się normalności żeby rozpoznawać prawdziwe anomalie
    n_normal_only = int(args.sequences * 0.6)
    n_anomaly     = args.sequences - n_normal_only

    print(f"  Normal-only sequences: {n_normal_only} (60%)")
    print(f"  Anomaly sequences:     {n_anomaly} (40%)")
